      ...
      <van-tab title="报名信息">
        <van-cell-group>
          ...
          <van-field
            ...
            :disabled="disabled"
          />
          <p style="font-size: 12px;padding:10px 5px 50px"><span style="color: #EF4F26">* </span>学员必须保证以上资料真实无误，如因虚报导致的后果，由学员自
            行负责。</p>
        </van-cell-group>
            ...
            <van-cell title="证书上传：">
              <template>
                <van-uploader :after-read="certificationUpload" :disabled="certaficationEdit">
                  <img src="../../assets/images/add_photo.png" alt="" v-if="!certification.mainPic" width="86"
                       height="68">
    ...
    methods: {
      ...
      save() {
        ...
        Object.keys(this.userInfo).map((key)=>{
          ...
          if (value) {
            if (this.isEmojiCharacter(value)) {
        ...
        if (this.active === 1) {
          ...
          if (!this.userInfo.name) {
            this.$toast('姓名不能为空');
            return;
          } else if (this.isEmojiCharacter(this.userInfo.name)){
            this.$toast('姓名不能包含表情');
            return;
